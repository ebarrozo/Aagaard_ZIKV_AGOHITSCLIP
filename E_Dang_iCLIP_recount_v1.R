...
cp *.counts.txt /media/jochum00/Aagaard_Raid2/ebarrozo/dang/agoclip

## on aagaardlab2
cd /media/jochum00/Aagaard_Raid2/ebarrozo/dang/agoclip
cp *.txt /home/ebarrozo/dang/recount/agoclip
...
cd /home/ebarrozo/dang/recount/agoclip
ls
head SRR7062770.1.duprm.sorted.htseq.counts.txt
tail SRR7062770.1.duprm.sorted.htseq.counts.txt
## only has ENSG and counts, no gene_id or gene_name

#rm -r DESeq2_analysis_v1
mkdir DESeq2_analysis_v1
cp *.txt /home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1

cd /home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1
ls
...
setwd("/home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1")
...
# View the data
df  ## confirmed all 6 samples were processed
...
## from metadata using sampleID and infection. Added an X in front of 6000 samples for R
meta<-as_tibble(read.csv("/home/ebarrozo/dang/recount/agoclip/dang.ago.meta.final.csv",sep = ",",header = T))
meta  ## 42 × 3
# Meta has SampleID, infection, rep
    ## infections are Mock, Paraiba, BaP, BbF, DBA, and Mixture
    ## 7 replicates each

#make a dataframe with a coaagaardhitsclipn that matches the names of all the count files in the results folder
df<-data.frame(file=list.files("/home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1"))
df
## remove list.tsv
...
# tmp<-as_tibble(read.table(file = paste0("/home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1/",df$Files[1],"_1.trimmed_PE.fastq.gz.duprm.sorted.htseq.counts.txt"),sep = "\t",header = F))
  tmp<-as_tibble(read.table(file = paste0("/home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1/",df$Files[1]),sep = "\t",header = F))
...
new.names <- read.csv(file="/home/ebarrozo/dang/recount/agoclip/new.names.csv", header=F)
colnames(new.names)<-"gene"

tmp<-as_tibble(read.table(file = paste0("/home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1/",df$Files[1]),sep = "\t",header = F))
tmp2<-as_tibble(read.table(file = paste0("/home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1/",df$Files[2]),sep = "\t",header = F))
tmp3<-as_tibble(read.table(file = paste0("/home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1/",df$Files[3]),sep = "\t",header = F))
tmp4<-as_tibble(read.table(file = paste0("/home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1/",df$Files[4]),sep = "\t",header = F))
tmp5<-as_tibble(read.table(file = paste0("/home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1/",df$Files[5]),sep = "\t",header = F))
tmp6<-as_tibble(read.table(file = paste0("/home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1/",df$Files[6]),sep = "\t",header = F))
...
# SRR7062770 SRR7062771 SRR7062772 SRR7062773 SRR7062774 SRR7062775 
 #   503483     259584      85060      11412      68308     106388 

col.sums <- colSums(df5)

#write the count matrix to a table
 write.table(x = df5,file = "aagaardhitsclip.count_matrix.tsv",sep = "\t",row.names = T)
...
df$...7
# df$Sample
df<-full_join(df,mds, by=c("SampleID"="Sample"))
df<-as_tibble(df)%>%
...
## save PCA plot 
png(file=paste0("PCA_plot_default-infection.png"),
                res=300, 
                width=2000, 
                height=1500)
a
...
plots
plots$sp<-plots$sp+
  stat_density_2d(aes(colour = infection,size=0.05),contour = T,alpha=0.2,size=0.05,
...
png(file=paste0("PCA_custom-plot.png"),
                ...
                height=1500)
plots
# ggpar(p = plots,legend.title = "Group") ## this command will change the legend from infection to whatever you want
dev.off()
...
# customise plot
customised_plot <- 
  ggplot(df,mapping = aes(x = Eig2,y = Eig1, z=0, color=infection)) +
  geom_point()+
  geom_contour() +
...
##   630 duplicate rownames were renamed by adding numbers
...
res <- results(dds, contrast=c("infection","Mock","Paraiba"), alpha=0.05, cooksCutoff=FALSE) ## Filtering adj. p < 0.05
summary(res)
    # 1 up and 1 down;  low counts [2]     : 0
...
## https://support.bioconductor.org/p/83626/
     ...
     plotPCA(vsd,intgroup=c("infection"))
     p1 <-      plotPCA(vsd,intgroup=c("infection"))
     p1
...
#########################################################################################################
##########################    Iteration I:    Mock-vs-Paraibainfection Analysis       ###########################################
#########################################################################################################
########## Determine the transcript counts for all samples to determine if there are any 0's or outliers. 
setwd("/home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1")
# load("/home/ebarrozo/dang/recount/agoclip/DESeq2_analysis_v1/all-PCA/aagaardhitsclip_data-v1.RData")

dir.create("Mock-vs-Paraibainfection")
setwd("Mock-vs-Paraibainfection")

# Differential expression analysis based on the Negative Binomial (a.k.a. Gamma-Poisson) distribution
dds <-DESeq(deseq_counts,parallel = T)
res <- results(dds, contrast=c("infection","Mock","Paraiba"), alpha=0.05, cooksCutoff=FALSE) ## Filtering adj. p < 0.05
summary(res)
  ## Whatever is last in the contrast option is what the FC value is attibuted to. Any DE with this analysis is attributed to the Paraiba 
  ## Wald hypothesis testing results in 63 up and 190 down significant genes. Also consider LTR 
write.csv(res, file = "Mockinfection_Paraiba_DESeq2_allResults-Wald.csv")
...
res.LTR <- results(dds.LTR, contrast=c("infection","Mock","Paraiba"), alpha=0.05, cooksCutoff=FALSE) ## Filtering adj. p < 0.05
summary(res.LTR)
write.csv(res.LTR, file = "Mockinfection_Paraiba_DESeq2_allResults-LTR.csv")
...
topResults
write.csv(topResults, file = "Mockinfection_Paraiba_DESeq2_topResults.csv")
write.csv(resOrdered, file = "Mockinfection_Paraiba_DESeq2_allResults-ordered.csv")

########## Generate a heatmap of the counts with only the Mock and Paraiba infection samples + the significant genes
...
res4<-full_join(res3,df%>%select(SampleID,infection)%>%distinct_all())
# res4<-right_join(res3,df, by="SampleID")%>%select(SampleID,infection)%>%distinct_all()
#res4<-right_join(res3,df, by="SampleID")%>%select(SampleID,infection)%>%distinct_all()

res4
res5<-res4%>%filter(infection%in%c("Mock","Paraiba"))%>%distinct_all()
res5
...
sig2<-full_join(sig,df%>%select(SampleID,infection)%>%distinct_all())
#sig2<-right_join(sig,df, by=c("SampleID"="...7"))%>%select(SampleID,infection,gene,count)%>%distinct_all()
sig2
sig3<-sig2%>%filter(infection%in%c("Mock","Paraiba"))%>%distinct_all()%>%
...
top.genes.Mock.Paraiba <- top.genes
...
dim(h)  # 20 12
df_slim<-df%>%
  select(SampleID,infection)%>%
  distinct_all()%>%
  filter(infection%in%c("Mock","Paraiba"))%>%
  distinct_all()
df_slim
df
anot_col<-data.frame(df_slim)%>%select(infection)
...
p1 <- pheatmap(mat = h,
         ...
         annotation_col = anot_col,
         scale = "none")
png(file=paste0("Mock-VS-Paraiba_counts-heatmap_row-scaled_notcut.png"),
                res=300, 
                width=3000, 
                height=2000)
p1
...
## PCA plot
pcaData <- plotPCA(vst, intgroup=c("infection"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
p2 <- ggplot(pcaData, aes(PC1, PC2, color=infection, shape=infection)) +
  geom_point(size=3) +
...
png(file=paste0("Mockinfection_Paraibainfection_countsplot-padj.png"),
                res=300, 
                width=2500, 
                height=1500)
plotCounts(dds, gene=which.min(res$padj), intgroup='infection')
dev.off()

############ Plot histogram with 1 gene. Can be modified to compare any gene of interest.
png(file=paste0("Mockinfection_Paraibainfection_countsplot-pvalue.png"),
                res=300, 
                width=2500, 
                height=1500)
plotCounts(dds, gene=which.min(res$pvalue), intgroup='infection')
dev.off()

### Shows dispersion of all data
png(file=paste0("Mockinfection_Paraibainfection_DispEsts.png"),
                res=300, 
                width=2500, 
                height=1500)
plotDispEsts(dds)
...
png(file=paste0("Mockinfection_Paraibainfection_p-hist.png"),
                res=300, 
                width=2500, 
                height=1500)
...
png(file=paste0("Mockinfection_Paraibainfection_padj-hist.png"),
                res=300, 
                width=2500, 
                height=1500)
...
png(file=paste0("Mockinfection_Paraibainfection_p-hist_smooth.png"),
                res=300, 
                width=2500, 
                height=1500)
...
sidecols <- c("grey","dodgerblue")[ vst$Paraiba ]
mat <- assay(vst)[ topVarGenes, ]
mat <- mat - rowMeans(mat)
colnames(mat) <- paste0(vst$Paraiba,"-",vst$infection)
...
png(file=paste0("Mockinfection_Paraibainfection_volcanoplot.png"),
                res=300, 
                width=1500, 
                height=1500)
...
png(file=paste0("Mockinfection_Paraibainfection_Enhancedvolcanoplot.png"),
                res=300, 
                width=2500, 
                height=2500)
p3
...
p3 <-  EnhancedVolcano(res,
    ...
    y = 'pvalue')
p3
png(file=paste0("Mockinfection_Paraibainfection_Enhancedvolcanoplot_top9.png"),
                res=300, 
                width=2500, 
                height=2500)
p3
...
p3 <-  EnhancedVolcano(res,
    ...
    y = 'pvalue')
p3
png(file=paste0("Mockinfection_Paraibainfection_Enhancedvolcanoplot_top.wo.tmsb15b.png"),
                res=300, 
                width=2500, 
                height=2500)
p3
