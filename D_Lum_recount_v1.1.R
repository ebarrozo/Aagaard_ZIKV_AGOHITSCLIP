...
## lum untouched htseq-outs are located here /home/ebarrozo/lum/results
cd /home/ebarrozo/lum/results
mkdir recount
cp *.counts.txt /home/ebarrozo/lum/results/recount
	## filter samples based on placentaldisc and cd45neg; see metadata file

################################################################################
##########################    Data Wrangling in Terminal      ######################################################
################################################################################
## in Terminal make a copy of the data in case there is an error
cd /home/ebarrozo/lum/results/recount
rm -r DESeq2_analysis_v1
mkdir DESeq2_analysis_v1
cp *.txt /home/ebarrozo/lum/results/recount/DESeq2_analysis_v1

cd /home/ebarrozo/lum/results/recount/DESeq2_analysis_v1
ls
...
## Column B- need to add gene names for ZIKV thru miRbase (36573 thru 39235) [36373:39235]
...
setwd("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1")
...
# View the data
df  ## confirmed all 42 samples were processed
# write.table(x = df,file = "samples.tsv",sep = "\t",row.names = T)

## RNA-seq filenames    KA-27566_1.trimmed_PE.fastq.gz.duprm.sorted.htseq.counts.txt
## RNA-seq filenames    SRR10318591_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt


# Clip file names
df<-df%>%mutate(name=gsub("_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt","",V1))%>%select(-V1)
df  
...
## from metadata using sampleID and Condition. Added an X in front of 6000 samples for R
meta<-as_tibble(read.csv("/home/ebarrozo/lum/docs/lum.meta.final.csv",sep = ",",header = T))
meta  ## 42 × 3
# Meta has SampleID, Condition, rep
    ## Conditions are Uninfected, ZIKV, BaP, BbF, DBA, and Mixture
    ## 7 replicates each

#make a dataframe with a column that matches the names of all the count files in the results folder
df<-data.frame(file=list.files("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1"))
...
# df2 <-right_join(df,meta, by=c("Sample"="SampleID"))
    ## right_join makes column$SampleID NAs
#df2 <- bind_cols(df,meta)
...
tmp<-as_tibble(read.table(file = paste0("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1/",df$SampleID[1],"_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt"),sep = "\t",header = F))
...
new.names <- read.csv(file="/home/ebarrozo/lum/docs/new.names.csv", header=F)
colnames(new.names)<-"gene"


tmp<-as_tibble(read.table(file = paste0("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1/",df$SampleID[1],"_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt"),sep = "\t",header = F))
tmp2<-as_tibble(read.table(file = paste0("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1/",df$SampleID[2],"_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt"),sep = "\t",header = F))
tmp3<-as_tibble(read.table(file = paste0("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1/",df$SampleID[3],"_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt"),sep = "\t",header = F))
tmp4<-as_tibble(read.table(file = paste0("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1/",df$SampleID[4],"_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt"),sep = "\t",header = F))
tmp5<-as_tibble(read.table(file = paste0("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1/",df$SampleID[5],"_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt"),sep = "\t",header = F))
tmp6<-as_tibble(read.table(file = paste0("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1/",df$SampleID[6],"_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt"),sep = "\t",header = F))
tmp7<-as_tibble(read.table(file = paste0("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1/",df$SampleID[7],"_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt"),sep = "\t",header = F))
tmp8<-as_tibble(read.table(file = paste0("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1/",df$SampleID[8],"_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt"),sep = "\t",header = F))
tmp9<-as_tibble(read.table(file = paste0("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1/",df$SampleID[9],"_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt"),sep = "\t",header = F))
tmp10<-as_tibble(read.table(file = paste0("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1/",df$SampleID[10],"_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt"),sep = "\t",header = F))
tmp11<-as_tibble(read.table(file = paste0("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1/",df$SampleID[11],"_1.trimmed_PE.fastq.duprm.sorted.htseq.counts.txt"),sep = "\t",header = F))
...
colnames(tmp10)<-c("gene",df$SampleID[10])
colnames(tmp11)<-c("gene",df$SampleID[11])
...
tmp10$gene<- new.names$gene
tmp11$gene<- new.names$gene
...
df3 <- full_join(df3,tmp10)
df3 <- full_join(df3,tmp11)
...
#write the count matrix to a table
 write.table(x = df4.1,file = "Lum.count_matrix.tsv",sep = "\t",row.names = T)
  ## YAY we have a counts matrix :)
   write.table(x = col.sums,file = "Lum.sum.counts.tsv",sep = "\t",row.names = T)
...
df$...8
# df$Sample
df<-full_join(df,mds, by=c("...8"="Sample"))
df<-as_tibble(df)%>%
...
## save PCA plot 
png(file=paste0("PCA_plot_default-Condition.png"),
                res=300, 
                width=2000, 
                height=1500)
a
...
plots
plots$sp<-plots$sp+
  stat_density_2d(aes(colour = Condition,size=0.05),contour = T,alpha=0.2,size=0.05,
...
png(file=paste0("PCA_custom-plot.png"),
                ...
                height=1500)
plots
# ggpar(p = plots,legend.title = "Group") ## this command will change the legend from Condition to whatever you want
dev.off()
...
# customise plot
customised_plot <- 
  ggplot(df,mapping = aes(x = Eig2,y = Eig1, z=0, color=Condition)) +
  geom_point()+
  geom_contour() +
...
res <- results(dds, contrast=c("Condition","Uninfected","ZIKV"), alpha=0.05, cooksCutoff=FALSE) ## Filtering adj. p < 0.05
summary(res)
...
save.image("lum_data-v1.RData")

setwd("..")
###### DESeq2 will have several iterations 
#########################################################################################################
##########################    Iteration I:    Uninfected-vs-ZIKVCondition Analysis       ###########################################
#########################################################################################################
########## Determine the transcript counts for all samples to determine if there are any 0's or outliers. 
setwd("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1")
# load("/home/ebarrozo/lum/results/recount/DESeq2_analysis_v1/all-PCA/lum_data-v1.RData")

dir.create("Uninfected-vs-ZIKVCondition")
setwd("Uninfected-vs-ZIKVCondition")
...
write.csv(res, file = "UninfectedCondition_ZIKV_DESeq2_allResults-Wald.csv")
...
res.LTR <- results(dds.LTR, contrast=c("Condition","Uninfected","ZIKV"), alpha=0.05, cooksCutoff=FALSE) ## Filtering adj. p < 0.05
summary(res.LTR)
write.csv(res.LTR, file = "UninfectedCondition_ZIKV_DESeq2_allResults-LTR.csv")
...
topResults
write.csv(topResults, file = "UninfectedCondition_ZIKV_DESeq2_topResults.csv")
write.csv(resOrdered, file = "UninfectedCondition_ZIKV_DESeq2_allResults-ordered.csv")

########## Generate a heatmap of the counts with only the Uninfected and ZIKV Condition samples + the significant genes
...
## res4<-full_join(res3,df%>%select(Sample,Condition,rep)%>%distinct_all())
res4<-right_join(res3,df, by=c("SampleID"="...8"))%>%select(SampleID,Condition)%>%distinct_all()
res4
res5<-res4%>%filter(Condition%in%c("Uninfected","ZIKV"))%>%distinct_all()
res5
...
# sig2<-full_join(sig,df%>%select(Sample,Condition)%>%distinct_all())
sig2<-right_join(sig,df, by=c("SampleID"="...8"))%>%select(SampleID,Condition,gene,count)%>%distinct_all()
sig2
sig3<-sig2%>%filter(Condition%in%c("Uninfected","ZIKV"))%>%distinct_all()%>%
...
df_slim<-df%>%
  select("...8",Condition)%>%
  distinct_all()%>%
  filter(Condition%in%c("Uninfected","ZIKV"))%>%
  distinct_all()
df_slim
df
anot_col<-data.frame(df_slim)%>%select(Condition)
rownames(anot_col)<-df_slim$"...8"
...
####### Default heatmap
pheatmap(mat = h,
         color = colorRampPalette(rev(brewer.pal(n = 11, name ="RdYlBu")))(13),
...
p2 <- ggplot(pcaData, aes(PC1, PC2, color=Condition, shape=Condition)) +
  geom_point(size=3) +
...
png(file=paste0("UninfectedCondition_ZIKVCondition_countsplot-padj.png"),
                res=300, 
                width=2500, 
                height=1500)
...
############ Plot histogram with 1 gene. Can be modified to compare any gene of interest.
png(file=paste0("UninfectedCondition_ZIKVCondition_countsplot-pvalue.png"),
                res=300, 
                width=2500, 
                height=1500)
...
png(file=paste0("UninfectedCondition_ZIKVCondition_DispEsts.png"),
                res=300, 
                width=2500, 
                height=1500)
plotDispEsts(dds)
...
png(file=paste0("UninfectedCondition_ZIKVCondition_p-hist.png"),
                res=300, 
                width=2500, 
                height=1500)
...
png(file=paste0("UninfectedCondition_ZIKVCondition_padj-hist.png"),
                res=300, 
                width=2500, 
                height=1500)
...
png(file=paste0("UninfectedCondition_ZIKVCondition_p-hist_smooth.png"),
                res=300, 
                width=2500, 
                height=1500)
...
colnames(mat) <- paste0(vst$ZIKV,"-",vst$Condition)
...
png(file=paste0("UninfectedCondition_ZIKVCondition_volcanoplot.png"),
                res=300, 
                width=1500, 
                height=1500)
...
png(file=paste0("UninfectedCondition_ZIKVCondition_Enhancedvolcanoplot.png"),
                res=300, 
                width=2500, 
                height=2500)
p3
...
p3 <-  EnhancedVolcano(res,
    ...
    y = 'pvalue')
p3
png(file=paste0("UninfectedCondition_ZIKVCondition_Enhancedvolcanoplot_top9.png"),
                res=300, 
                width=2500, 
                height=2500)
p3
...
p3 <-  EnhancedVolcano(res,
    ...
    y = 'pvalue')
p3
png(file=paste0("UninfectedCondition_ZIKVCondition_Enhancedvolcanoplot_top.wo.tmsb15b.png"),
                res=300, 
                width=2500, 
                height=2500)
p3
